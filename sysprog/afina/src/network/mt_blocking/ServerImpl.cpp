...
void ServerImpl::Join() {
    ...
    {
        std::unique_lock<std::mutex> lock(_main_mutex);
...
// See Server.h
    void ServerImpl::OnRun() {
        while (running.load()) {
            ...
            {
                std::lock_guard<std::mutex> lock(_main_mutex);
                if (_workers.size() >= _max_connections) {
                    close(client_socket);
                    continue;
                }

                _workers.emplace(client_socket, std::thread(&ServerImpl::_connection_processing, this, client_socket));
            }
        }
        _logger->warn("Network stopped");
    }

    void ServerImpl::_connection_processing(int client_socket) {
